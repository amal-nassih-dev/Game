...
function setProgress() {
  const total = (typeof flowTotalSteps === "function") ? flowTotalSteps() : 26;
  const val = `Step ${Math.min(stepIndex, total)} / ${total}`;
...
// Fasting-aware energy/mood helpers
...
function pickBodyAwareActivity(type) {
  ...
  let activities = [];
  if (type === "break") activities = bodyInfo.breakActivities;
...
function getCorrectActivityDuration(activityType) {
  switch (activityType) {
    case "break": return getBreakDurationByMood();
    case "pause": return getPauseDurationByMood();
    case "grounding":
    case "silence": return 2;
    case "morningAudio": return 5;
    case "quranMemo": return 15;
    case "quranReading": return 15;
    case "writing": return 30;
    case "research": return 30;
    case "lunch": return appConfig.fasting && isBeforeIftar() ? 10 : 25;
    default: return 5;
  }
}
...
// Floating "Next" helpers used by timer end
